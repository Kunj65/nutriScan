...
export const HoverEffect = ({
  ...
  return (
    <div className={cn("grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 py-10", className)}>
      {items.map((item, idx) => (
        <a
          href={item?.link}  // Using <a> instead of Link
          key={item?.link}
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-[#118AB2] dark:bg-slate-800/[0.8] block rounded-3xl"
...
export const CardTitle = ({
  ...
  return (
    <h4 className={cn("text-black font-bold text-2xl tracking-wide mt-4", className)}>
      {children}
    </h4>
  );
};
